...
module.exports.execute = () => {
  let dir = new File(File.IMPEX + "/src/catalogRobert/products");
  let final = new File(File.IMPEX + "/src/catalog/finalProductsRoberts.xml");
  let finalWritter = new FileWriter(final);
  let startFile =
    '<?xml version="1.0" encoding="UTF-8"?><catalog xmlns="http://www.demandware.com/xml/impex/catalog/2006-10-31" catalog-id="storefront-catalog-roberts"><header><image-settings><internal-location base-path="/" /><view-types><view-type>large</view-type><view-type>medium</view-type><view-type>small</view-type><view-type>swatch</view-type></view-types><alt-pattern>${productname}</alt-pattern><title-pattern>${productname}</title-pattern></image-settings></header>';
