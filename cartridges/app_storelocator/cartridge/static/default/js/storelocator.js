...
function initMap() {
  ...
  updateStores(map, storesArray);
  createDetails(map);
  initAutocomplete(map, storesArray);
...
function handleClickUserLocation(map, stores, isClick = false) {
  if (currentPositionClient.lat && currentPositionClient.lng) {
    ...
    const error = (e) => {
      ...
      if (isClick) {
        alert(userLocationIcon.getAttribute("location-disable-message"));
      }
    };
...
function initAutocomplete(map, stores) {
...
function setFilters(map, stores, { lat, lng }) {
  const storeWDistance = stores.map((store) => ({
    ...store,
    distance: calDistance(store.latitude, store.longitude, lat, lng).toFixed(2),
  }));

  const filterStores = storeWDistance
    .filter(({ distance }) => distance < MAX_DISTANCE)
    .sort((a,b) => {if(a.state < b.state) return -1})
