 ...
 */
server.get('UpdateGrid', cache.applyShortPromotionSensitiveCache, consentTracking.consent, function (req, res, next) {
    ...
    if (req.querystring.productLayout == 2) {
        res.render('/search/productGrid', {
            productSearch: productSearch
        });
    }
    if (req.querystring.productLayout == 4) {
        res.render('/search/mobile/productGrid', {
            productSearch: productSearch
        });
    }
...
server.get('mobileRefinebar', cache.applyDefaultCache, function (req, res, next) {
    var CatalogMgr = require('dw/catalog/CatalogMgr');
    var ProductSearchModel = require('dw/catalog/ProductSearchModel');
    var ProductSearch = require('*/cartridge/models/search/productSearch');
    var searchHelper = require('*/cartridge/scripts/helpers/searchHelpers');
    ...
    res.render('/search/mobile/mobilesearchRefineBar', {
        productSearch: productSearch,
        querystring: req.querystring
    });
...
server.get('ShowAjax', cache.applyShortPromotionSensitiveCache, consentTracking.consent, function (req, res, next) {
    ...
    let template = '/search/searchResultsNoDecorator';
    if (req.querystring.productLayout == 2) {
        template = '/search/searchResultsNoDecorator';
    }
    if (req.querystring.productLayout == 4) {
        template = '/search/mobile/searchResultsNoDecorator';
    }
    // res.json({
