...
server.append('SelectShippingMethod', server.middleware.https, function (req, res, next) {
    ...
    var ShippingHelper = require('*/cartridge/scripts/checkout/shippingHelpers');
    var COHelpers = require("*/cartridge/scripts/checkout/checkoutHelpers");
    var selectedShippingMethod = ShippingHelper.getAllShipmentsForShippingMethod(req.querystring.methodID || req.form.methodID);
    ...
    session.privacy.defaultSelectedShipment = null;

    if (selectedShippingMethod.custom.storePickupEnabled) {
        session.privacy.defaultSelectedShipment = selectedShippingMethod.ID;
        ShippingHelper.markShipmentForShipping(shipment);
        ShippingHelper.markAllShipmentsForShippingMethod(selectedShippingMethod.ID, currentBasket);
        var viewData = res.getViewData();
        ...
        //if there are no ship to home shipments, copy the first shipment to default shipment and remove the new shipment
        if (currentBasket.shipments.length > 1) {
            ShippingHelper.markMultipleShipmentsToOne(currentBasket);
        }
        ShippingHelper.markAllShipmentsForShippingMethod(selectedShippingMethod.ID, currentBasket);
    }
