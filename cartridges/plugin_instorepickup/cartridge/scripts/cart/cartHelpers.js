...
function addProductToCart(currentBasket, productId, quantity, childProducts, options, storeId, req) {
    ...
    if (productInCart) {
        ...
        // setting by dafult shipment to pickup in-store based on pickstore method selection on checkout
        if (!empty(session.privacy.defaultSelectedShipment)) {
            var ShippingHelper = require('*/cartridge/scripts/checkout/shippingHelpers');
            ShippingHelper.markAllShipmentsForShippingMethod(session.privacy.defaultSelectedShipment, currentBasket);
        }
